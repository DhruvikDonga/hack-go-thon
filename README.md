...
- 🔔 **Mobile Webhook Notification Subsystem (`/api/v1/webhooks/*`)**  
  Built for mobile app push relays and third-party event consumers. Allows registering webhook URLs with topic filtering (`events: ["*"]` or `["notification", "alert"]`), cryptographic payload signing (`X-Webhook-Signature: sha256=<hmac>`), non-blocking asynchronous dispatch, live ping testing (`POST /api/v1/webhooks/test`), delivery logs, and automatic notification dispatch to a dedicated `webhooks` room on the `simplysocket` WebSocket mesh (accessible by all accounts except standard user-level, requiring Auth Level > 1). Backed by PostgreSQL (`webhook_subscriptions` & `webhook_delivery_logs`) with automatic schema migration and fallback to in-memory mode for offline standalone operation.
